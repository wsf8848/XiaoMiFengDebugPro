...
void My_USART3_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		My_USART3_SendByte(Array[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void My_USART3_SendString(char *String)
{
	...
	{
		My_USART3_SendByte(String[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：返回值等于X的Y次方
  */
uint32_t My_USART3_Pow(uint32_t X, uint32_t Y)
{
...
void My_USART3_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		My_USART3_SendByte(Number / My_USART3_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
{
	My_USART3_SendByte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
void My_USART3_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	My_USART3_SendString(String);		//串口发送字符数组（字符串）
}
